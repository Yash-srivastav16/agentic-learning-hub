...
function normaliseState(rawState) {
  ...
  return {
    ...
    lastVisitedPath: typeof state.lastVisitedPath === "string" ? state.lastVisitedPath : "./index.html"
  };
}
...
function resetLearningProgress(state) {
  ...
  state.startedAt = "";
  state.updatedAt = new Date().toISOString();
  state.lastVisitedPath = "./index.html";
  saveState(state);
}
...
function visitCurrentPage(state, meta) {
  ...
  if (meta.type === "lesson" && meta.lesson) {
    ...
    state.lastVisitedPath = `./${meta.fileName}`;
  }
...
function syncSidebarStatuses(state) {
  document.querySelectorAll('.sidebar-link[href^="./lesson-"]').forEach((link) => {
...
function ensureSidebarUtilityLinks(meta) {
  ...
  document.querySelectorAll(".sidebar-link").forEach((link) => {
    ...
    link.classList.toggle("active", fileName === meta.fileName);
  });
}
...
function renderTopbarControls(state, meta) {
  ...
  if (info.completedCount >= TOTAL_LESSONS) {
    const certificateButton = createActionLink("Certificate", "./certificate.html", "button-primary");
...
function renderQuickTools(state) {
  ...
  return `
    <div class="lms-quick-tools">
      <a class="button button-secondary" href="./index.html">Overview</a>
      <a class="button button-secondary" href="./revision.html">Revision</a>
      <a class="button button-secondary" href="./glossary.html">Glossary</a>
      <a class="button button-secondary" href="./course-summary.html">Summary</a>
      <a class="button button-secondary" href="./certificate.html">${info.completedCount >= TOTAL_LESSONS ? "Certificate" : "Preview certificate"}</a>
    </div>
  `;
}
...
function renderTrackerCard(state, meta) {
  ...
  if (info.furthestLesson > 0) {
    ...
    if (info.nextLesson) {
      ...
      actions.append(createActionLink("Open Summary", "./course-summary.html", "button-secondary"));
      actions.append(createActionLink("Start again from Lesson 1", "./lesson-1.html", "button-secondary"));
      actions.append(createActionLink("Open Certificate", "./certificate.html", "button-secondary"));
    }
  } else {
    actions.append(createActionLink("Start Lesson 1", "./lesson-1.html", "button-primary"));
    actions.append(createActionLink("Open Course Map", "./index.html", "button-secondary"));
  }
  ...
  wrapper.innerHTML = `
    ...
    <article class="card lms-lesson-nav-card" style="margin-top: 16px;">
      ...
      <div class="lms-tracker-actions">
        ...
        ${nextLesson ? `<a class="button button-primary" href="${nextLesson.href}">Next Lesson ${nextLesson.number}</a>` : `<a class="button button-primary" href="./course-summary.html">Open Summary</a>`}
        ${nextLesson ? `<a class="button button-secondary" href="./revision.html">Open Revision</a>` : `<a class="button button-secondary" href="./certificate.html">Open Certificate</a>`}
      </div>
    </article>
  `;
...
function renderDashboard(state, meta) {
  ...
  if (info.furthestLesson > 0) {
    ...
    if (info.nextLesson) {
      ...
      actionSlot.append(createActionLink("Open Summary", "./course-summary.html", "button-secondary"));
      actionSlot.append(createActionLink("Start Lesson 1 Again", "./lesson-1.html", "button-secondary"));
      actionSlot.append(createActionLink("Open Certificate", "./certificate.html", "button-secondary"));
    }
    const resetButton = document.createElement("button");
    resetButton.type = "button";
    ...
    actionSlot.append(createActionLink("Start Lesson 1", "./lesson-1.html", "button-primary"));
    actionSlot.append(createActionLink("Open Glossary", "./glossary.html", "button-secondary"));
  }
...
function updateOverviewActions(state) {
  ...
  if (primary) {
    primary.href = info.furthestLesson > 0 ? info.continueLesson.href : "./lesson-1.html";
  ...
  if (secondary) {
    if (info.nextLesson) {
      ...
      secondary.href = "./certificate.html";
  ...
  function renderEntries(filter = "") {
    ...
    grid.innerHTML = entries.map((entry) => `
      <article class="card lms-glossary-card">
        ...
        <div class="lms-chip-list">
          ${entry.lessons.map((lessonNumber) => `<a class="lms-study-chip" href="./lesson-${lessonNumber}.html">Lesson ${lessonNumber}</a>`).join("")}
        </div>
      </article>
    `).join("");
      ...
      <div class="two-up">
        ...
        <article class="card">
          ...
          <div class="lms-dashboard-actions">
            <a class="button button-primary" href="${info.furthestLesson > 0 ? info.continueLesson.href : "./lesson-1.html"}">${info.furthestLesson > 0 ? `Continue Lesson ${info.furthestLesson}` : "Start Lesson 1"}</a>
  ...
  panel.innerHTML = `
    ...
    <div class="lms-certificate">
      ...
      <div class="lms-certificate-actions">
        ...
        <a class="button button-secondary" href="./course-summary.html">Open summary</a>
      </div>
    </div>
  `;
      ...
      <div class="lms-celebration-actions">
        ${
          milestone.nextLesson
            ? `<a class="button button-primary" href="./lesson-${milestone.nextLesson}.html">Start Lesson ${milestone.nextLesson}</a>`
            : `<a class="button button-primary" href="./certificate.html">Open certificate</a><a class="button button-secondary" href="./course-summary.html">Open course summary</a>`
        }
